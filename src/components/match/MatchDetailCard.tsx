...
export function MatchDetailCard({
  ...
  // Live spectator link: any player of a not-yet-closed match. (The RPC
  // also allows plain game members; the card keeps it to players.)
  const canLiveShare =
    viewerIsPlayer &&
    match.status !== "completed" &&
    match.status !== "cancelled"
  ...
  // ── Live spectator link (public /live/[token]) ─────────────
  // Enable is idempotent server-side; the button shares (or copies) the
  // public URL and offers a prefilled WhatsApp intent — the link is meant
  // to be dropped into the group chat as the round starts.
  const handleLiveShare = async (via: "native" | "whatsapp") => {
    setLiveShareBusy(true)
    try {
      const { data, error: rpcError } = await supabase.rpc("enable_live_share", {
        p_match_id: match.id,
      })
      if (rpcError) throw rpcError
      const res = data as { success: boolean; token?: string; error?: string }
      if (!res.success || !res.token) return
      const url = `${window.location.origin}/live/${res.token}`
      track("live_share_enabled", { match_id: match.id })
      if (via === "whatsapp") {
        const msg = t("live.whatsapp.message", { url })
  ...
  const handleLiveShareDisable = async () => {
    try {
      await supabase.rpc("disable_live_share", { p_match_id: match.id })
      await onRefresh()
    } catch {
      /* surface nothing — the danger zone re-renders from parent state */
    }
  }
      ...
      {!editing &&
        (canEnterScores ||
          ...
          <div className="mt-3 flex flex-col gap-2">
            ...
            {/* Live spectator link (Phase: live share) — WhatsApp-first. */}
            {canLiveShare && (
              <div className="flex items-stretch gap-2">
                <button
                  type="button"
                  disabled={liveShareBusy}
                  onClick={() => handleLiveShare("native")}
                  className="flex h-9 flex-1 items-center justify-center gap-1.5 rounded-lg border border-red-200 bg-red-50 px-2.5 text-xs font-semibold text-red-600 hover:bg-red-100 disabled:opacity-60"
                >
                  <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-red-500" />
                  {liveCopied ? t("games.match.linkcopied") : t("live.share.cta")}
                </button>
                <button
                  type="button"
                  disabled={liveShareBusy}
                  onClick={() => handleLiveShare("whatsapp")}
                  aria-label={t("live.share.whatsapp")}
                  title={t("live.share.whatsapp")}
                  className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-[#25D366] text-white hover:opacity-90 disabled:opacity-60"
                >
            ...
            {canLiveShare && match.live_share_token && (
              <button
                type="button"
                onClick={handleLiveShareDisable}
                className="self-start text-[11px] text-primary/40 underline-offset-2 hover:underline"
              >
                {t("live.share.disable")}
              </button>
            )}
